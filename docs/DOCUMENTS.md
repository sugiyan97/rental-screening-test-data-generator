...
- **赤字（色）マイナスのバリアント**（`financial_statement/us_gaap_en`、Issue #80）— 損益計算書の金額が `-` または `(` で始まる場合、そのセルに `.negative`（赤色）クラスを自動付与する（`(case.financials.xxx or '').startswith(('-', '('))` による条件分岐。値そのものは通常どおり著者が文字列で入力し、テンプレート側は色を付けるだけ）。色情報はPDFのテキスト抽出では失われるため、符号記号（-／括弧）だけに依存しない検知ロジックの穴を突く検証用（CASE-ML-000025 参照）。既存の黒字ケース（CASE-ML-000001 等）は正の値のみのため出力に影響しない
- **¥記号のJPY/CNY曖昧性**（`bank_balance_certificate/cn_mainland_deposit_certificate`、Issue #80）— 存款证明书の金額を「元」ではなく「¥」記号で表記し、日本円の「¥」表記と紙面上は区別が付かないケース（CASE-ML-000024）。正解 JSON 側の `source_currency` は常に `CNY` を明示するため採点は曖昧にならない
- **通貨・数値表記の横断テスト観点の一覧** — 各多言語variantがどの観点（通貨記号の曖昧性・桁区切り／マイナス表記・単位倍率）を担うかは [CASES.md の J節](CASES.md) を参照
