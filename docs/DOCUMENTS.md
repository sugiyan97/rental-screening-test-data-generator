...
本ツールが生成できる書類は 32 種類の `document_type`、合計 73 種類の variant。
...
| `financial_statement` | 決算書風（財務サマリー） | `financial_summary`, `financial_summary_prior`, `multi_period`, `multi_period_report_form`, `us_gaap_en`, `singapore_hk_en`, `ifrs_consolidated_en`, `kr_nts_standard`, `kr_kifrs_audited`, `kr_sme_simple`, `kr_kgaap_bracket_minus` |
| `trial_balance` | 合計残高試算表風 | `monthly_summary`, `annual_summary` |
| `business_opening_notice` | 個人事業の開業・廃業等届出書（開業届）写し風 | `individual` |
| `bank_balance_certificate` | 預貯金残高証明書風（金融機関発行） | `standard`, `standard_en`, `standard_en_scan_degraded`, `kr_standard` |
...
- **個人申込書(standard)の申込種類欄** — `case.property.application_type`（「事務所」「店舗」「倉庫」「貸地」「工場」）で `rental_application_individual/standard` の「申込種類」欄のチェックが出し分けられる。未設定時は5択すべて `□` のまま表示される。star-link-review-integrator側 #2572 ガード（チェック欄が無ければnullとし他の記載内容から推測しない）対策として追加した項目のため、必ずチェック欄を伴う形で値を設定すること（`usage_purpose` の自由記述だけでは申込種類は抽出されない）
...
- **預貯金残高証明書** — 金融機関発行の残高証明書風。新規法人・新規個人事業で自己資金の証明に使用（英語版 `bank_balance_certificate/standard_en`、韓国語版 `bank_balance_certificate/kr_standard`（잔액증명서）あり）。`kr_standard` は `amount_in_words`（한글大字金額。例:「금 일십이억사천오백육십만원정」）を設定すると金額欄の下に併記される（未設定なら従来どおり出力されない）
...
- **スキャン画質劣化・公印重なりバリアント**（`bank_balance_certificate/standard_en_scan_degraded`、Issue #76 P0）— 実書類で「公印が金額に重なる低画質スキャンでは桁スケール誤り・数字の読み違い・フィールド欠損が発生する」ことを確認済みのパターンを再現する。SVG `feTurbulence` によるノイズテクスチャ・`contrast`/`brightness` フィルタでのコントラスト低下・半透明で回転した円形の「公印（ダミー）」を残高金額に重ねる演出を CSS/SVG のみで実装しており、新規のPython依存や画像処理パイプラインは追加していない（`standard_en` と同じデータをそのまま使い、演出のみを上乗せしたvariant）
- **通貨単位が数字非隣接のバリアント**（`financial_statement/singapore_hk_en`、Issue #76 P0）— 通貨・単位表記（例: `HK$'000`）を見出し直下に1回だけ記載し、表中の金額数字自体には通貨記号を一切付与しないレイアウト。「数字に隣接する通貨表記」だけを前提にした外貨検知ロジックの穴を突く検証用
